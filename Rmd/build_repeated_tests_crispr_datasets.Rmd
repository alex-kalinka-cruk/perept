...
output:
  html_document:
    ...
    toc_float: true
params:
  out: "~/data/az_cruk/perept_data/ht29-iorio"
  fdr: !r 0.1
---
...
* Use an FDR of 0.1 to define significant genes.
...
process_mageck_output <- function(file_list){
  ...
  for(file in file_list){
    ...
    ts <- read.table(file, header=T, stringsAsFactors = F) %>%
      ...
      rowwise() %>%
      mutate(neg_or_pos_hit = ifelse((neg_hit==1 || pos_hit==1),1,0)) %>%
      ungroup() %>%
...
files <- list.files(file.path(params$out,"output","mageck"), pattern = "gene_summary.txt$", full.names = T)
ht29.mageck_hit_matrix <- process_mageck_output(files)

ht29.mageck_neg <- ht29.mageck_hit_matrix %>%
  select(id, contains("neg_hit")) %>%
  mutate(pos_count = rowSums(.[2:13]))
